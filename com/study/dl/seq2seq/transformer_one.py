...
class EncoderLayer(nn.Module):
    def __init__(self, head, ffn,dim=512, q=64):
        super(EncoderLayer, self).__init__()
        self.multi_attention = nn.ModuleList([SelfAttentio(dim=dim, q=q)
                                              for i in range(head)])
    ...
    def forward(self, input):
        ...
        multi_attention = t.cat(attentions, dim=2)  # multi_atten:batch,seq,512
        z = self.Wz(multi_attention)
        attention_output = F.normalize((z + input), dim=2)
        ffn_output = self.ffn(attention_output)

        output = F.normalize(ffn_output + attention_output)
        return output

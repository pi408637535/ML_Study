...
class MultiHeadAttention(nn.Module):
    ...
    #设计失误啊,对于Encoder来说:Q,K,V都来自Encoder但对于Decoder来说Q来自Decoder，K,V来自Encoder
    def forward(self, input, atten_mask):
        #batch,seq,embed: batch,seq,512
        batch = input.shape[0]
        Q = self.Wq(input)  #batch,seq,64 * 8
        K = self.Wk(input) #batch,seq,64 * 8
        V = self.Wv(input) #batch,seq,64 * 8
        ...
        return nn.LayerNorm(512)(Z + input),atten
...
class DecoderLayer(nn.Module):
    def __init__(self,self_att_mode, ffn):
        ...
        self.enc_deco_attn = self_att_mode
        self.ffn = ffn

    def forward(self, deco_inputs, deco_self_attn_mask):
        #deco_inputs:batch,seq,dim:batch,seq,512
        #deco_self_attn_mask:batch,seq,seq
        enc_outputs, attn = self.deco_self_attn(deco_inputs,
                                               deco_self_attn_mask)
...
if __name__ == '__main__':
    ...
    enc_outputs, enc_self_attens = encoder(data)
